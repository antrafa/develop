...
database.execute('delete from books where id = 3');

try{
	let dados = database.execute('select id, name, age from author where id = 1');
	let dados2 = database.execute('select name, authorId, isbn from books where id = 1');
